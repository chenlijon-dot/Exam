(() => {
  ...
  function decodeBase64Parts(parts) {
    const normalized = parts.map(normalizeBase64);

    // Case 1: files are slices of one continuous Base64 stream.
    const joined = normalized.join('');
    try {
      return binaryStringToBytes(atob(joined));
    } catch (joinedError) {
      // Case 2: every part was Base64-encoded separately. In that case
      // '=' padding may appear in the middle of the concatenated string,
      // which Android/Chrome atob() correctly rejects.
      try {
        return concatBytes(normalized.map(part => binaryStringToBytes(atob(part))));
      } catch (partError) {
        // Last-resort compatibility path for padded chunks that represent
        // one stream but were concatenated with interior '=' characters.
        try {
          let compact = joined.replace(/=/g, '');
          while (compact.length % 4) compact += '=';
          return binaryStringToBytes(atob(compact));
        } catch (compactError) {
          console.error('[vocab-memory] base64 decode failed', {
            joinedError, partError, compactError,
            lengths: normalized.map(x => x.length),
            tails: normalized.map(x => x.slice(-8))
          });
          throw new Error('字庫編碼資料損壞，請重新整理後再試。');
        }
      }
    }
  }

  async function decodeCompressedVocabulary(parts) {
    if (typeof DecompressionStream !== 'function') {
      throw new Error('此瀏覽器不支援字庫解壓縮，請使用最新版 Chrome / Edge。');
    }
    const bytes = decodeBase64Parts(parts);
  ...
  async function loadVocabulary() {
    ...
    vocabularyPromise = (async () => {
      ...
      const rows = await decodeCompressedVocabulary(parts);
      const filtered = rows.filter(row => row?.id && row?.w && row?.n && row?.c);
      if (!filtered.length) throw new Error('找不到可用字彙');
      console.info(`[vocab-memory] loaded ${filtered.length} vocabulary rows`);
      return filtered;
    })().catch(error => {

(() => {
  ...
  function showMath71Unit(unitKey) {
    ...
    $('#catalogContent').innerHTML = `
      ...
      <p class="catalog-sub">課本單元起始頁 p.${unit.page}；1-1、1-2 已建立分級自編題庫，其餘小節依教材知識庫逐步建立。</p>
      <div class="catalog-grid">
        ${unit.sections.map(section => {
          ...
          <button class="catalog-card chapter-card" data-math-section="${section.code}" ${ready ? '' : 'disabled'}>
            ...
            <span class="desc">課本起始頁 p.${section.page}｜${ready ? (section.code === '1-2' ? '簡易 20 題・中等 10 題・困難 10 題｜含數線距離與等距題' : '簡易 20 題・中等 10 題・困難 10 題') : '題庫待建'}</span>
          </button>`;
        }).join('')}
      </div>`;
